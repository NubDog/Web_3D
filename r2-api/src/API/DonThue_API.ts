...
// Danh sách tất cả các cột hợp lệ trong bảng DonThue để xác thực
const ALL_COLUMNS = [
    'don_thue_id',
    'khach_hang_id',
    'phuong_tien_id',
...
/**
 * Xử lý yêu cầu GET để lấy dữ liệu từ bảng DonThue.
 * Cho phép tùy chọn các cột cần lấy thông qua query parameter 'fields'.
 */
export async function handleGetDonThue(request: Request, env: Env): Promise<Response> {
    ...
    const query = `SELECT ${columnsToSelect} FROM DonThue`;
